...
function notFound(req, res, next) { res.status(404); const error = new Error('Not Found - ' + req.originalUrl); next(error); }
;
function errorHandler(err, res) { res.status(res.statusCode || 500); res.json({ message: err }); }
;
app.use(notFound);
