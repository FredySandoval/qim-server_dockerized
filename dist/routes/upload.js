...
router.post('/', async (req, res) => {
    ...
    if (content_length === 0 || !("files" in req))
        res.status(400).send('Request body cannot be emtpy');
    const req_file_keys = Object.keys(req.files);
    if (!req_file_keys.includes(CLIENT_FILES))
        return res.status(400).send('Invalid request 01');
...
function handleUploadFiles(req, res) {
    let upload_path;
    let file_unique_name;
    let file_extension;
